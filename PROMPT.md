...
scored against 31 days of real Claude Code session transcripts (4,596 Bash
...
- **Noise.** With no allow-list, routine `grep` / `go build` / `git diff` calls
  prompt constantly on a personal account.
...
Target: a **personal** Claude account, not a managed one.
...
caught the call that deleted a memory folder.
...
```bash
cd /Users/me/.claude/projects/…/memory && rm -f MEMORY.md
```
...
`cwd` = `~/Workspace/example-repo`, `HOME` = `/Users/me`.
...
| `git stash` (all forms) | `sub` | 7 | The stack is shared across worktrees. |
...
| with `--force` or `-f` | ask — 0/mo. **`--force-with-lease` is allowed**: it refuses if the remote moved, and it is a standard PR-update workflow (16/mo) |
...
```json
{"ts":"2026-09-07T14:22:19Z","session":"5d6c09b1","cwd":"/Users/…/example-repo",
...
- **Back up first** to `settings.json.bouncer-<timestamp>.bak`. Note
  `settings.json.bak` may already exist and belong to something else
...
1. **Unit**: `go test -race ./...`. `internal/shellwalk` and `internal/rules`
   ...
   git push --force-with-lease origin feature-branch  -> allow
   ...
   git push --force origin feature-branch             -> ask
...
4. On a managed account a hook `allow` cannot beat a managed `deny` or
...
- **Do not run `enable` / `disable` yourself.** `~/.claude/settings.json` is
  write-denied in the sandbox, and going live is the owner's call.
